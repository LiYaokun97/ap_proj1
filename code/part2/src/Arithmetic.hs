...
-- return if a expression is wrong
wrongExp :: Either ArithError Integer -> Bool
wrongExp (Right _) = False
wrongExp (Left _) = True

-- helper function for    evalErr (Pow e1 e2) env
-- For two Either type, return whether Right x < Right y by compare x and y 
...
-- first, compute if from and to is error-free 
-- second, compare from and to cause f should be no larger than t
-- third, try if there is error in body when calculating using value from [f..t]
-- if so, return the error, otherwise get the final result 
